...
if sys.platform == 'win32':
    ...
    plat_ext = [
                Extension("Crypto.Random.OSRNG.winrandom",
                          ...
                          include_dirs=[get_file_path() + 'src/'],
                          sources=[get_file_path() + "src/winrand.c"])
               ]
else:
    HTONS_LIBS = []
    plat_ext = []
...
def get_configure_path():
    return "%s/configure" % get_file_path()
...
class PCTBuildExt (build_ext):
    ...
    def detect_modules (self):
        ...
        if self.compiler.compiler_type == 'msvc':
            ...
            self.compiler.include_dirs.insert(0, get_file_path() + "src/inc-msvc/")
            ac = self.__read_autoconf(get_file_path() + "src/inc-msvc/config.h")
        else:
...
class PCTBuildConfigure(Command):
    ...
    def run(self):
        ...
        v_cmd1 = "chmod 0755 %s" % get_configure_path()
            if os.system(v_cmd1) != 0:
                raise RuntimeError("chmod error")
            v_cmd2 = "sh %s" % get_configure_path() # we use "sh" here so that it'll work on mingw32 with standard python.org binaries
            if self.verbose < 1:
                v_cmd2 += " -q"
            if os.system(v_cmd2) != 0:
...
kw = {'name':"pycrypto",
      ...
      'packages' : ["Crypto", "Crypto.Hash", "Crypto.Cipher", "Crypto.Util",
                  ...
                  "Crypto.PublicKey",
                  "Crypto.Signature"],
      'package_dir': {"Crypto": get_file_path() + "/lib/Crypto"},
      'ext_modules': plat_ext + [
            # _fastmath (uses GNU mp library)
            Extension("Crypto.PublicKey._fastmath",
                      include_dirs=[get_file_path() + '/src/', '/usr/include/'],
                      libraries=['gmp'],
                      sources=[get_file_path() + "/src/_fastmath.c"]),

            # Hash functions
            Extension("Crypto.Hash._MD2",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + "/src/MD2.c"]),
            Extension("Crypto.Hash._MD4",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + "/src/MD4.c"]),
            Extension("Crypto.Hash._SHA256",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + "/src/SHA256.c"]),
            Extension("Crypto.Hash._SHA224",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + "/src/SHA224.c"]),
            Extension("Crypto.Hash._SHA384",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + "/src/SHA384.c"]),
            Extension("Crypto.Hash._SHA512",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + "/src/SHA512.c"]),
            Extension("Crypto.Hash._RIPEMD160",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + "/src/RIPEMD160.c"],
                      define_macros=[endianness_macro()]),

            # Block encryption algorithms
            Extension("Crypto.Cipher._AES",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + "/src/AES.c"]),
            Extension("Crypto.Cipher._ARC2",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + "/src/ARC2.c"]),
            Extension("Crypto.Cipher._Blowfish",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + "/src/Blowfish.c"]),
            Extension("Crypto.Cipher._CAST",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + "/src/CAST.c"]),
            Extension("Crypto.Cipher._DES",
                      include_dirs=[get_file_path() + '/src/', get_file_path() + '/src/libtom/'],
                      sources=[get_file_path() + "/src/DES.c"]),
            Extension("Crypto.Cipher._DES3",
                      include_dirs=[get_file_path() + '/src/', get_file_path() + '/src/libtom/'],
                      sources=[get_file_path() + "/src/DES3.c"]),

            # Stream ciphers
            Extension("Crypto.Cipher._ARC4",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + "/src/ARC4.c"]),
            Extension("Crypto.Cipher._XOR",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + "/src/XOR.c"]),

            # Utility modules
            Extension("Crypto.Util.strxor",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + '/src/strxor.c']),

            # Counter modules
            Extension("Crypto.Util._counter",
                      include_dirs=[get_file_path() + '/src/'],
                      sources=[get_file_path() + '/src/_counter.c']),
    ]
}
